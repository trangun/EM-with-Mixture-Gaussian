...
def get_output(filename):
    ...
    if filename.endswith('_em_gaussian.py'):
        cmd = 'python {} --nodev --iterations 1 --clusters_file gaussian_smoketest_clusters.txt --data_file {} --print_params'.format(filename, os.path.join(DATA_DIR,'points.dat'))
    else:
        cmd = 'python {} --nodev --iterations 1 --clusters_file aspect_smoketest_clusters.txt --data_file {} --print_params'.format(filename, os.path.join(DATA_DIR,'pairs.dat'))
